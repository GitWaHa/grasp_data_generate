...
class DetectGraspTwoStageServer
{
private:
    ros::NodeHandle nh_;
    // ros::Publisher pub_;
...
public:
    DetectGraspTwoStageServer()
    {
        server_ = nh_.advertiseService("/grasp_detecter/get_pose", &DetectGraspTwoStageServer::serverCallBack, this);
    }
    ~DetectGraspTwoStageServer() {}
...
{
    ros::init(argc, argv, "detect_grasps_two_stage_server");

    DetectGraspTwoStageServer calculatet_pose;
    ros::Duration(1).sleep();
    ROS_INFO("detect_grasps_two_stage_server server is started");
    ros::spin();

    return 0;
}

bool DetectGraspTwoStageServer::serverCallBack(grasp_detect::GraspDetect::Request &req, grasp_detect::GraspDetect::Response &res)
{
