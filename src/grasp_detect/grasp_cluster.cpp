...
namespace grasp_detect
{
    ...
    std::vector<std::unique_ptr<candidate::Hand>> GraspCluster::findClusters(
        const std::vector<std::unique_ptr<candidate::Hand>> &hand_list)
    {
        ...
        for (int i = 0; i < hand_list.size(); i++)
        {
            ...
            if ((is_remove_inliers_ && has_used[i]))
                continue;

            for (int j = 0; j < hand_list.size(); j++)
            {
                if (i == j || (is_remove_inliers_ && has_used[j]))
                    continue;
