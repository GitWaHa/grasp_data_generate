...
namespace grasp_detect
{
    namespace util
    {
        ...
        void Cloud::randomSample(int num)
        {
            pcl::RandomSample<pcl::PointXYZRGBA> rs;
            rs.setInputCloud(cloud_processed_);
            //设置输出点的数量
            rs.setSample(num);

            //下采样并输出到cloud_out
            rs.filter(*cloud_processed_);
        }
        ...
        void Cloud::removeNans()
        {
            ...
            if (inliers->indices.size() < cloud_processed_->size())
            {
                ...
                printf("[Cloud] Cloud after removing NANs: %zu\n", cloud_processed_->size());
            }
            else
            {
                printf("[Cloud] Cloud after removing NANs: 0 \n");
            }
        }
        ...
        void Cloud::removeStatisticalOutliers()
        {
            ...
            printf("[Cloud] Cloud after removing statistical outliers: %zu\n", cloud_processed_->size());
        }
        ...
        void Cloud::refineNormals(int k)
        {
            ...
            printf("[Cloud] Refining surface normals ...\n");
            printf("[Cloud] mean: %.3f, max: %.3f, sum: %.3f\n", diff.mean(), diff.maxCoeff(),
                   diff.sum());
        ...
        void Cloud::filterWorkspace(const std::vector<double> &workspace)
        {
            ...
            if (sample_indices_.size() > 0)
            {
                ...
                std::cout << "[Cloud]" << sample_indices_.size()
        ...
        void Cloud::voxelizeCloud(float cell_size)
        {
            ...
            printf("[Cloud] Voxelized cloud: %zu\n", cloud_processed_->size());
        }

        void Cloud::subsample(int num_samples)
        {
            std::cout << "[Cloud] num_samples is " << num_samples << std::endl;
            if (num_samples == 0)
                return;
        ...
        void Cloud::subsampleUniformly(int num_samples)
        {
            ...
            std::cout << "[Cloud] subsampleUniformly " << sample_indices_.size() << std::endl;
        }

        void Cloud::subsampleSamples(int num_samples)
        {
            std::cout << "[Cloud] subsampleSamples " << num_samples << std::endl;
            ...
            {
                std::cout << "[Cloud]"
                          << "Using " << num_samples << " out of " << samples_.cols()
                          << " available samples.\n";
        ...
        {
            std::cout << "[Cloud] subsampleSampleIndices " << num_samples << std::endl;
        ...
        void Cloud::sampleAbovePlane()
        {
            ...
            printf("[Cloud] Sampling above plane ...\n");
            ...
            {
                sample_indices_ = indices;
                printf("[Cloud] Plane fit succeeded. %zu samples above plane.\n",
                       sample_indices_.size());
            }
            else
            {
                printf("[Cloud] Plane fit failed. Using entire point cloud ...\n");
            }
            std::cout << "[Cloud] runtime (plane fit): " << omp_get_wtime() - t0 << "\n";
        }
        ...
        void Cloud::calculateNormals(int num_threads, double radius)
        {
            ...
            printf("[Cloud] Calculating surface normals ...\n");
            std::string mode;
            ...
            printf("[Cloud] Calculated %zu surface normals in %3.4fs (mode: %s).\n",
                   normals_.cols(), t_gpu, mode.c_str());
            printf(
                "[Cloud] Reversing direction of normals that do not point to at least one camera "
                "...\n");
            reverseNormals();
        }
        ...
        void Cloud::calculateNormalsOMP(int num_threads, double radius)
        {
            ...
            for (int i = 0; i < view_points_.cols(); i++)
            {
                ...
                printf("[Cloud] runtime(computeNormals): %3.4f\n", omp_get_wtime() - t0);
                normals_list[i] = normals_cloud;
                printf("[Cloud] camera: %d, #indices: %d, #normals: %d \n", i,
        ...
        void Cloud::reverseNormals()
        {
            ...
            std::cout << " [Cloud] reversed " << c << " normals\n";
            std::cout << " [Cloud] runtime (reverse normals): " << omp_get_wtime() - t1 << "\n";
        }
