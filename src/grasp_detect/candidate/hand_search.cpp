...
namespace grasp_detect
{
    namespace candidate
    {
        ...
        HandSearch::HandSearch(const grasp_detect::util::HandSearchParams &params)
            : params_(params), plots_local_axes_(false)
        {
            ...
            antipodal_ = std::make_unique<Antipodal>(params.friction_coeff_,
                                                     params.min_viable_);
            ...
            const util::Cloud &cloud_cam) const
        {
            ...
            std::cout << "[HandSearch] Estimating local reference frames ...\n";
            ...
            {
                std::cout << "[HandSearch] Error: No samples or no indices!\n";
            ...
            {
                plot_->plotLocalAxes(frames, cloud_cam.getCloudProcessed());
            }

            // 2. Evaluate possible hand placements.
            std::cout << "[HandSearch] Finding hand poses ...\n";
            std::vector<std::unique_ptr<HandSet>> hand_set_list = evalHands(cloud_cam, frames, kdtree);

            const double t2 = omp_get_wtime();
            std::cout << "[HandSearch] ====> HAND SEARCH TIME: " << t2 - t0_total << std::endl;
...
#pragma omp parallel for private(nn_indices, nn_dists, nn_points) \
    num_threads(params_.num_threads_)
            ...
            const pcl::KdTreeFLANN<pcl::PointXYZRGBA> &kdtree) const
        {
            ...
            int num_orientations = params_.num_orientations_;
            if (num_orientations % 2 == 0) //必须是奇数，这样才能有０度
                num_orientations++;

            const Eigen::VectorXd angles_space = Eigen::VectorXd::LinSpaced(
                num_orientations, -1.0 * M_PI / 6, M_PI / 6);

            // necessary b/c assignment in Eigen does not change vector size
            Eigen::VectorXd angles(params_.num_orientations_);
            if (params_.num_orientations_ == 1)
                angles(0) = 0.0;
            else
                angles = angles_space.head(params_.num_orientations_);
            std::cout << "[HandSearch] angles:" << angles << std::endl;
...
#pragma omp parallel for private(nn_indices, nn_dists, nn_points) \
    num_threads(params_.num_threads_)
            ...
            printf("[HandSearch] Found %d hand sets in %3.2fs\n", (int)hand_set_list.size(),
                   omp_get_wtime() - t1);
